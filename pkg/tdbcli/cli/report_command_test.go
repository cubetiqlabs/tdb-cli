...
func TestBuildReportBodyAggregatesAndGroupBy(t *testing.T) {
	...
	for _, spec := range agg {
		...
		if op == "sum" && spec["alias"] != "total_sales" {
			t.Fatalf("missing alias for sum: %#v", spec)
		}
	}
}

func TestDecideStreamingExport(t *testing.T) {
	if ok, _ := decideStreamingExport(true, nil, false, "jsonl"); !ok {
		t.Fatalf("expected streaming true")
	}
	if ok, reason := decideStreamingExport(true, []string{"f=1"}, false, "jsonl"); ok || reason == "" {
		t.Fatalf("expected filter rejection")
	}
	if ok, _ := decideStreamingExport(true, nil, true, "jsonl"); ok {
		t.Fatalf("expected includeDeleted rejection")
	}
	if ok, _ := decideStreamingExport(true, nil, false, "json"); ok {
		t.Fatalf("expected json format rejection")
	}
}
